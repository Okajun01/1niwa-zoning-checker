...
# タブ切り替え
tab1, tab2, tab6, tab4, tab5, tab7, tab3 = st.tabs(["📝 住所を入力", "📄 CSVで一括チェック", "📧 メールから物件チェック", "🔍 物件検索", "🏚️ 空き家バンク", "📰 業界ニュース", "ℹ️ 用途地域の説明"])
...
with tab7:
    st.subheader("📰 業界ニュース")
    st.markdown("旅館業・宿泊業界の最新ニュースを自動収集しています。法改正・規制変更を見逃さないようチェックしましょう。")

    try:
        from news_collector import load_news, CATEGORIES, IMPORTANCE_LEVELS, update_impact_memo

        news_data = load_news()
        articles = news_data.get("articles", [])
        last_updated = news_data.get("last_updated", "未取得")
        ...
        if articles:
            ...
            filtered = [
                a for a in articles
                if a.get("category") in cat_filter and a.get("importance") in imp_filter
            ]

            # 重要度サマリー
            high = sum(1 for a in filtered if a.get("importance") == "高")
            mid = sum(1 for a in filtered if a.get("importance") == "中")
            low = sum(1 for a in filtered if a.get("importance") == "低")
            col1, col2, col3 = st.columns(3)
            col1.metric("🔴 重要", f"{high}件")
            col2.metric("🟡 通常", f"{mid}件")
            col3.metric("🔵 参考", f"{low}件")

            st.markdown("---")

            # ニュース一覧
            for i, article in enumerate(filtered):
                imp = article.get("importance", "中")
                imp_icon = {"高": "🔴", "中": "🟡", "低": "🔵"}.get(imp, "⚪")
                date = article.get("date", "")
                title = article.get("title", "（タイトルなし）")
                category = article.get("category", "")

                with st.expander(f"{imp_icon} [{date}] {title}　— {category}", expanded=(imp == "高")):
                    st.markdown(f"**要約**: {article.get('summary', '（要約なし）')}")

                    if article.get("source"):
                        st.markdown(f"**情報源**: {article['source']}")

                    if article.get("url"):
                        st.markdown(f"[元記事を読む]({article['url']})")

                    # 影響メモ表示
                    memo = article.get("impact_memo", "")
                    if memo:
                        st.info(f"**1NIWAへの影響**: {memo}")
                    ...
                    new_memo = st.text_input(
                        ...
                        value=memo,
                        key=f"memo_{i}",
                        placeholder="例: 23区の許可基準が変更される可能性あり",
                    )
                    if new_memo != memo and st.button("メモ保存", key=f"save_memo_{i}"):
                        url = article.get("url", "")
                        if url and update_impact_memo(url, new_memo):
